...
async function SpecCon(scr, cdecode = cb => [0xffeeeeee, 0xff202020]) {
...
class SpecKbd extends SoftKbd {
    constructor(kbd_elem, con, con_elem, mx = false) {
        super(kbd_elem, con, con_elem, true);
        this.mx = mx;
        this.ports = new Uint8Array([0xff, 0xff, 0xff, 0]);
    }
    translateKey(e, soft) {
        switch (e.key) {
            case 'АР2':    return this.mx ? [0xff, 0x7f, 0xf7] : [0xbf, 0xfb, 0xff]; // АР2
            case 'КОИ':    return [0xff, 0x7f, 0xfb]; // КОИ
            case 'F1':     return this.mx ? [0xff, 0x7f, 0xfd] : [0xff, 0x7f, 0xf7]; // F1  F
            case 'F2':     return this.mx ? [0xff, 0x7f, 0xfe] : [0xff, 0x7f, 0xfb]; // F2  HELP
            case 'F3':     return this.mx ? [0x7f, 0x7f, 0xff] : [0xff, 0x7f, 0xfd]; // F3  NEW
            case 'F4':     return this.mx ? [0xbf, 0x7f, 0xff] : [0xff, 0x7f, 0xfe]; // F4  LOAD
            case 'F5':     return this.mx ? [0xdf, 0x7f, 0xff] : [0x7f, 0x7f, 0xff]; // F5  SAVE
            case 'F6':     return this.mx ? [0xef, 0x7f, 0xff] : [0xbf, 0x7f, 0xff]; // F6  RUN
            case 'F7':     return this.mx ? [0xf7, 0x7f, 0xff] : [0xdf, 0x7f, 0xff]; // F7  STOP
            case 'F8':     return this.mx ? [0xfb, 0x7f, 0xff] : [0xef, 0x7f, 0xff]; // F8  CONT
            case 'F9':     return this.mx ? [0xfd, 0x7f, 0xff] : [0xf7, 0x7f, 0xff]; // F9  EDIT
            case 'СТР':    return [0xfe, 0x7f, 0xff]; // СТР
            case '\u25a0': return [0xfb, 0x7f, 0xff]; // СФ
            case '\u25a1': return [0xfd, 0x7f, 0xff]; // ТФ
            case '\u2341': return [0xfe, 0x7f, 0xff]; // СТР
    ...
    processKey(val) {
        this.ports[3] = this.down ? 1 : 0;
        for (let i = 0; i < 3; i++)
            if (this.down) this.ports[i] &= val[i];
            else this.ports[i] |= ~val[i];
    }
}
...
async function main() {
    const loads = [loadScript('js/js8080.js'), loadScript('js/hrdwr8.js')],
          clrs04 = [0xffeeeeee, 0xff2020ff, 0xff20ff20, 0xffff2020],
          ...
          s1 = mxkb ? `
<div class='key key_emul'>АР2</div><div class='key key_emul'>КОИ</div>
<div class='key key_emul'>F1</div><div class='key key_emul'>F2</div>
<div class='key key_emul'>F3</div><div class='key key_emul'>F4</div>
<div class='key key_emul'>F5</div><div class='key key_emul'>F6</div>
<div class='key key_emul'>F7</div><div class='key key_emul'>F8</div>
<div class='key key_emul'>F9</div><div class='key key_emul'>СТР</div>
<div class='sp3'></div>
          ` : `
<div class='key key_emul'>F1</div><div class='key key_emul'>F2</div>
<div class='key key_emul'>F3</div><div class='key key_emul'>F4</div>
<div class='key key_emul'>F5</div><div class='key key_emul'>F6</div>
<div class='key key_emul'>F7</div><div class='key key_emul'>F8</div>
<div class='key key_emul'>F9</div><div class='key key_emul i'>&#x25a0;</div>
<div class='key key_emul i'>&#x25a1;</div><div class='key key_emul i'>&#x2341;</div>
<div class='sp3'></div>
          `,
          s2 = mxkb ? `
<div class='key key_emul sp4'>&#x0020;</div>
          ` : `
<div class='key key_emul'>АР2</div><div class='key key_emul'>&#x0020;</div>
          `,
          [scr_elem, kbd_elem, con_elem] = createUI(
              addTab('emul', 'EMULATOR', 1, true),
              'emul', 'emu', '36px', 27, 6, 'calc(36px / 10)', '800px', '480px', ``, `
<div class='section sec_emul sec_emul_left'>
    ${s1}
    <div class='key key_emul'><span>;</span><span>+</span></div>
    <div class='key key_emul'><span>1</span><span>!</span></div>
    <div class='key key_emul'><span>2</span><span>"</span></div>
    <div class='key key_emul'><span>3</span><span>#</span></div>
    <div class='key key_emul'><span>4</span><span>$</span></div>
    <div class='key key_emul'><span>5</span><span>%</span></div>
    <div class='key key_emul'><span>6</span><span>&</span></div>
    <div class='key key_emul'><span>7</span><span>'</span></div>
    <div class='key key_emul'><span>8</span><span>(</span></div>
    <div class='key key_emul'><span>9</span><span>)</span></div>
    <div class='key key_emul'>0</div>
    <div class='key key_emul'><span>-</span><span>=</span></div>
    <div class='sp3'></div><div></div>
    <div class='key key_emul'><span>J</span><span>Й</span></div>
    <div class='key key_emul'><span>C</span><span>Ц</span></div>
    <div class='key key_emul'><span>U</span><span>У</span></div>
    <div class='key key_emul'><span>K</span><span>К</span></div>
    <div class='key key_emul'><span>E</span><span>Е</span></div>
    <div class='key key_emul'><span>N</span><span>Н</span></div>
    <div class='key key_emul'><span>G</span><span>Г</span></div>
    <div class='key key_emul'><span>[</span><span>Ш</span></div>
    <div class='key key_emul'><span>]</span><span>Щ</span></div>
    <div class='key key_emul'><span>Z</span><span>З</span></div>
    <div class='key key_emul'><span>H</span><span>Х</span></div>
    <div class='key key_emul'><span>:</span><span>*</span></div>
    <div class='sp2'></div><div class='sp2'></div>
    <div class='key key_emul'><span>F</span><span>Ф</span></div>
    <div class='key key_emul'><span>Y</span><span>Ы</span></div>
    <div class='key key_emul'><span>W</span><span>В</span></div>
    <div class='key key_emul'><span>A</span><span>А</span></div>
    <div class='key key_emul'><span>P</span><span>П</span></div>
    <div class='key key_emul'><span>R</span><span>Р</span></div>
    <div class='key key_emul'><span>O</span><span>О</span></div>
    <div class='key key_emul'><span>L</span><span>Л</span></div>
    <div class='key key_emul'><span>D</span><span>Д</span></div>
    <div class='key key_emul'><span>V</span><span>Ж</span></div>
    <div class='key key_emul'><span>\\</span><span>Э</span></div>
    <div class='key key_emul'><span>.</span><span>></span></div>
    <div></div><div class='sp3'></div>
    <div class='key key_emul'><span>Q</span><span>Я</span></div>
    <div class='key key_emul'><span>^</span><span>Ч</span></div>
    <div class='key key_emul'><span>S</span><span>С</span></div>
    <div class='key key_emul'><span>M</span><span>М</span></div>
    <div class='key key_emul'><span>I</span><span>И</span></div>
    <div class='key key_emul'><span>T</span><span>Т</span></div>
    <div class='key key_emul'><span>X</span><span>Ь</span></div>
    <div class='key key_emul'><span>B</span><span>Б</span></div>
    <div class='key key_emul'><span>@</span><span>Ю</span></div>
    <div class='key key_emul'><span>,</span><span><</span></div>
    <div class='key key_emul'><span>/</span><span>?</span></div>
    <div class='key key_emul'><span>_</span><span>Ъ</span></div>
    <div class='key key_emul sp3'>НР</div><div class='key key_emul'>Р/Л</div>
    <div class='key key_emul i'>&#x21d6;</div><div class='key key_emul i'>&#x2191;</div>
    <div class='key key_emul i'>&#x2193;</div><div class='key key_emul'>ТАБ</div>
    ${s2}<div class='key key_emul i'>&#x2190;</div><div class='key key_emul'>ПВ</div>
    <div class='key key_emul i'>&#x2192;</div><div class='key key_emul'>ПС</div>
    <div class='key key_emul'>ВК</div>
</div>`);
    await Promise.all(loads);
    const con = await SpecCon(scr_elem, ccb),
...
//    await mon.exec('r 0 spec/loderun.bin'); // game for 8 colors, G0
    console.log(emu.loadBin( // sound test
        new Uint8Array(await (await preLoadFile('spec/xtree.rks')).arrayBuffer()).slice(4),
        0x0000
    ));                      // sound test G6
    mon.exec('g c000');
/*    await mon.exec('r d400 spec/TEST.I80');                // ryumik's test
...
/*    await mon.exec('r 0 spec/Specimx.rom'); // RamFOS
//    await mon.exec('r 0 spec/nc.rom');      // MXOS
    mem.fdc.Disk[1] = new Uint8Array(await (await preLoadFile('spec/bst_mx0.odi')).arrayBuffer());
    mem.fdc.Disk[0] = new Uint8Array(await (await preLoadFile('spec/LafanS2.odi')).arrayBuffer());
