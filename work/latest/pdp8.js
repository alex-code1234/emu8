...
// emu.html?js=pdp8.js&fields=0..7&terms=0..3&fpanel=0..2&run=0..1&os=os8|tss8|edu20|focal
    ...
    async handler(parms, cmd) {
        ...
        try { switch (cmd) {
            ...
            case 'os8copy':  // OS8 copy external file to disk
                ...
                break;
            case 'pal':      // load PAL binary listing
                if (parms.length < 2) { console.error('missing fname'); break; }
                console.log(this.emu.loadPAL(await loadFile(parms[1], true)));
                break;
...
this.exec('g 200');
                        break;
                    case 'd0ab':
await this.exec('tape pdp8/MAINDEC-8E-D0AB-pb.bpt'); await this.exec('x pc 200 sr 7777');
this.exec('g');
                        break;
                    case 'd0bb':
await this.exec('tape pdp8/MAINDEC-8E-D0BB-pb.bpt'); await this.exec('x pc 200'); this.exec('g');
                        break;
                    case 'd0gc':
await this.exec('tape pdp8/MAINDEC-8E-D0GC-pb.bpt'); await this.exec('x pc 200 if 0'); this.exec('g');
                        break;
                    case 'd0fc':
await this.exec('tape pdp8/MAINDEC-8E-D0FC-pb.bpt'); await this.exec('x pc 200 if 0');
...
await this.exec('m 266 240 240 324 240 0 0 0 0 215 212 215 215 317 240 0 0 0 0'); this.exec('g');
                        break;
                    case 'd0cc':
await this.exec('tape pdp8/MAINDEC-8E-D0CC-pb.bpt'); await this.exec('x pc 200 if 0 sr 0400');
...
await this.exec('tape pdp8/MAINDEC-8E-D1FB-pb.bpt'); await this.exec('x pc 200 sr 0002');
this.exec('g');
                        break;
                    case 'd1eb':
await this.exec('tape pdp8/MAINDEC-08-D1EB-pb.bpt'); await this.exec('x pc 200'); this.exec('g');
                        break;
                    case 'd0db':
await this.exec('tape pdp8/MAINDEC-8E-D0DB-pb.bpt'); await this.exec('x pc 200 if 0'); this.exec('g');
                        break;
                    case 'd0eb':
await this.exec('tape pdp8/MAINDEC-8E-D0EB-pb.bpt'); await this.exec('x pc 200 if 0'); this.exec('g');
                        break;
                    case 'd0jc':
await this.exec('tape pdp8/MAINDEC-8E-D0JC-pb.bpt'); await this.exec('x pc 200 if 0'); this.exec('g');
                        break;
                    case 'dhmca':
                        if (parms.length < 3) { console.error('missing TSE [0|1]'); break; }
await this.exec('tape pdp8/MAINDEC-08-DHMCA-A-pb.bpt');
                        if (parms[2] | 0) {
...
await this.exec('tse 0'); await this.exec('x pc 200 sr 4001');
                    ...
                    case 'vc8e': // point plotter test, sr = 0..7
                        if (parms.length > 2 && pi(parms[2])) {
                            await this.exec('pal pdp8/snowflk.ls');
                            await this.exec('x pc 200');
                        } else {
...
'0000 3236 7404 7041 3237 1236 7100 7510 7020 7010 2237 5227 5621 0000 0000');
                            await this.exec('x pc 200 sr 0000');
                        } this.exec('g');
                        break;
            ...
            case 'conn':     // connect device
                ...
                let sys_tab = document.getElementById('sys_tab'),
                    sys = document.getElementById('system'),
                    prm1lc = parms[1].toLowerCase(),
                    extra1 = prm1lc === 'vc8e' && parms.length > 2 && pi(parms[2]);
                if (sys_tab === null && ('rx01 rs08 kc8e'.indexOf(prm1lc) >= 0 || extra1)) {
                    sys = addTab('sys_tab', 'SYSTEM');
                    sys_tab = document.getElementById('sys_tab');
                    sys.id = 'system';
                }
                switch (prm1lc) {
                    case 'rx01': // RX8E/RX01 disk drive
                        if (this.fdd) { console.error('RX01 already connected'); break; }
                        await loadScript('pdp8/rx01.js');
                    ...
                    case 'kc8e': // KC8-E front panel
                        ...
                        await loadScript('pdp8/kc8_e.js');
                    ...
                    case 'vt':   // VT-52 CRT terminal
                        ...
                        if (parms[1].toLowerCase() === 'asr') {
                            ...
                            await loadScript('pdp8/vt_52.js');
                    ...
                    case 'vc8e': // VC8-E point plotter
                        ...
                        await loadScript('pdp8/vc8_e.js');
...
async function main() {
    await loadScript('pdp8/pdp_8e.js');
    const getNum = (p, e) => {
    ...
    if (os !== null) {                           // pre-defined configs
        ...
        if (os === 'tss8' || os === 'edu20') {
            ...
            if (os === 'tss8') {
                ...
                await mon.exec('tape pdp8/tss8_init.bin');           // prepare TSS/8 start
                await mon.exec('rs08 pdp8/tss8_rf.dsk');
                await mon.exec('x if 2 ib 2 pc 4200');
            } else {
                if (cores < 1) throw new Error('expected: min 1 ext field (8Kb total)');
                await mon.exec('tape pdp8/edu20c.pt');               // prepare Edu-20 start
            ...
            await mon.exec('r pdp8/os8boot3.oct 1');                 // prepare OS/8 start
            await mon.exec('rx01 0 pdp8/os8_rx.rx01');
            await mon.exec('x pc 22');
        }
        else if (os === 'focal') {
            await mon.exec('tape pdp8/FOCAL-69.bpt');                // prepare FOCAL-69 start
