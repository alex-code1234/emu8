...
class SM_1803 extends MemIO {
    constructor(con) {
        ...
        this.p03 = 0x00;                    // ports timeout bits
    ...
    input(p) {
        switch (p) {
            ...
            case 0x03: // system status
                ...
                this.p03 &= 0xf9; // reset 1, 2 timeout bits after read
                return res03;
            default:
...
async function SM_1800_7201_out(scr_elem) {
    const KOI7N1 = [
    ...
    GRAPH = [
        0x2593, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69,
        0x2518, 0x2510, 0x250c, 0x2514, 0x253c, 0x6f, 0x70,
        0x2500, 0x72, 0x73,
        0x251c, 0x2524, 0x2534, 0x252c, 0x2502
    ],
    fmap = (n, i) => {
        if (n === 1) return (i < 0x40 || i > 0x7f) ? i : KOI7N1[i - 0x40];
        if (n === 2) return (i < 0x61 || i > 0x78) ? i : GRAPH[i - 0x61];
        return i;
    };
    const con = await createCon(scr_elem, green, 'VT220', 80, 24, undefined, 'VT220', fmap),
          font2 = await con.loadFont(2, 'VT220'),
          lat_key = document.getElementById('kbd_ind'),
          old_disp = con.display;
    con.print('^[?25l^[2J^[?25h'); // clear screen and restore cursor
    let lat = true, in_esc = false, in_pos = 0;
    con.display = ccode => { // override to process VT52 esc codes and DEL (DOS1800) key
        ...
        if (ccode === 0x0e || ccode === 0x0f) { con.ruslat(ccode); return; }
        if (in_pos > 0) {
            in_pos--;
            ...
            case 0x24:        // ESC $ - cursor to column 0 (CR)
                if (in_esc) { // send ESC [ 80 D - cursor back 80 columns
                    old_disp(0x5b); old_disp(0x38); old_disp(0x30); ccode = 0x44;
                    in_esc = false;
                }
                break;
            ...
            case 0x45:
                ...
                break;
            case 0x46: case 0x47: // switch graphics mode
                if (!in_esc) break;
                in_esc = false;
                con.setFont((ccode === 0x46) ? font2 : lat ? 0 : 1);
                return;
    ...
    con.ruslat = code => {
        lat = (code === undefined) ? !lat : code === 0x0f;
        lat_key.style.color = lat ? 'inherit' : '#ffff00';
        con.print(`^${lat ? '(' : ')'}`);
        return lat ? 0x0f : 0x0e;
    };
    return con;
}

// VTA 2000-30 terminal input
class SM_1800_7201_in extends SoftKbd {
    constructor(kbd_elem, con, con_elem) {
        super(kbd_elem, con, con_elem);
    }
    trnCtrls(txt) {
        return (txt === 'ФР') ? 1 : (txt === 'СПС') ? 2 : (txt === 'УС') ? 3 : 0;
    }
    translateKey(e, soft) {
        if (soft) switch (e.key) {
            case '\u259f': return 255; // clear screen
            case '\u2590': return 254; // clear line
            case '\u21b0': return 2;   // home
            case '\u00a0': return 32;
            case 'АР2': return 27;
            case 'ВК': return 13;
            ...
            case 'ЗБ': return 0x7f;
            case 'ПС': return 10;
            case 'РУСЛАТ': return this.con.ruslat();
        }
        if (e.key.length > 1) {
            const key = e.key.charAt(0);
            if (key >= 'А' && key <= 'Я') e.key = e.key.charAt(1);
        }
        return super.translateKey(e, soft);
    }
    processKey(val) {
        if (!this.fs_ctrl) switch (val) {
            case 2: super.processKey(27); val = 0x48; break;   // home
            case 3: val = 14; break;                           // page down ^N (hw kbd)
            case 4: super.processKey(27); val = 0x43; break;   // right
            case 5: super.processKey(27); val = 0x41; break;   // up
            case 18: val = 16; break;                          // page up   ^P (hw kbd)
            case 19: super.processKey(27); val = 0x44; break;  // left
            case 24: super.processKey(27); val = 0x42; break;  // down
            case 255: super.processKey(27); val = 0x4a; break; // clear screen
            case 254: super.processKey(27); val = 0x4b; break; // clear line
        }
    ...
    async handler(parms, cmd) {
        try { switch (cmd) {
            ...
            case 'disk': // insert/save disk
                ...
                if (parms.length < 3) {
                    const dd = this.emu.memo.DRIVES[dn];
                    if (dd === null) { console.log('empty drive'); break; }
...
<div class='section sec_emul sec_emul_left'>
    <div class='key key_emul ksp'>АР2</div>
    ...
    <div class='key key_emul'><span>=</span><span>-</span></div>
    <div class='key key_emul ksp sp3'>ВШ</div><div class='ksp sp2'></div>
    <div class='key key_emul ksp sp3'>ГТ</div>
    <div class='key key_emul'><span>Й</span><span>J</span></div>
    <div class='key key_emul'><span>Ц</span><span>C</span></div>
    <div class='key key_emul'><span>У</span><span>U</span></div>
    <div class='key key_emul'><span>К</span><span>K</span></div>
    <div class='key key_emul'><span>Е</span><span>E</span></div>
    <div class='key key_emul'><span>Н</span><span>N</span></div>
    <div class='key key_emul'><span>Г</span><span>G</span></div>
    <div class='key key_emul'><span>Ш</span><span>[</span></div>
    <div class='key key_emul'><span>Щ</span><span>]</span></div>
    <div class='key key_emul'><span>З</span><span>Z</span></div>
    <div class='key key_emul'><span>Х</span><span>H</span></div>
    <div class='key key_emul'><span>*</span><span>:</span></div>
    <div class='key key_emul ksp'>ПС</div><div class='key key_emul ksp'>ВК</div>
    <div class='key key_emul ksp kctrl'>УС</div>
    <div class='key key_emul ksp'><span id='kbd_ind'>РУС</span><span>ЛАТ</span></div>
    <div class='key key_emul'><span>Ф</span><span>F</span></div>
    <div class='key key_emul'><span>Ы</span><span>Y</span></div>
    <div class='key key_emul'><span>В</span><span>W</span></div>
    <div class='key key_emul'><span>А</span><span>A</span></div>
    <div class='key key_emul'><span>П</span><span>P</span></div>
    <div class='key key_emul'><span>Р</span><span>R</span></div>
    <div class='key key_emul'><span>О</span><span>O</span></div>
    <div class='key key_emul'><span>Л</span><span>L</span></div>
    <div class='key key_emul'><span>Д</span><span>D</span></div>
    <div class='key key_emul'><span>Ж</span><span>V</span></div>
    <div class='key key_emul'><span>Э</span><span>\\</span></div>
    <div class='key key_emul'><span>></span><span>.</span></div>
    <div class='key key_emul ksp sp3'>ЗБ</div>
    <div class='ksp sp3'></div><div class='key key_emul ksp kshft'>СПС</div>
    <div class='key key_emul'><span>Я</span><span>Q</span></div>
    <div class='key key_emul'><span>Ч</span><span>^</span></div>
    <div class='key key_emul'><span>С</span><span>S</span></div>
    <div class='key key_emul'><span>М</span><span>M</span></div>
    <div class='key key_emul'><span>И</span><span>I</span></div>
    <div class='key key_emul'><span>Т</span><span>T</span></div>
    <div class='key key_emul'><span>Ь</span><span>X</span></div>
    <div class='key key_emul'><span>Б</span><span>B</span></div>
    <div class='key key_emul'><span>Ю</span><span>@</span></div>
    <div class='key key_emul'><span><</span><span>,</span></div>
    <div class='key key_emul'><span>?</span><span>/</span></div>
    <div class='key key_emul'><span>Ъ</span><span>_</span></div>
    <div class='key key_emul ksp kshft'>СПС</div>
    <div class='sp9'></div><div class='key key_emul sp14'>&nbsp;</div>
</div>
<div class='section sec_emul sec_emul_right'>
    <div class='key key_emul ksp i'>&#9631;</div><div class='key key_emul ksp i'>&#8593;</div>
    <div class='key key_emul ksp i'>&#9616;</div><div class='key key_emul ksp i'>&#8595;</div>
    <div class='key key_emul ksp i'>&#8624;</div><div class='key key_emul ksp i'>&#8594;</div>
    <div class='key key_emul ksp'>ФР</div><div class='key key_emul ksp i'>&#8592;</div>
</div>`, 31),
    getNum = (p, e) => {
    ...
    const con = await SM_1800_7201_out(scr_elem),
          mem = new SM_1803(con),
    ...
    if (monid) {
        await mon.exec('rom sm1800/MONID.ROM');                     // MONID 1.3
        mem.add(SM_5635_10(mem), [0x70, 0x71, 0x72, 0x73, 0x77]);   // FDC SM5615
    } else {
        await mon.exec('rom sm1800/MONID10.ROM');                   // MONID 1.0
        mem.add(SM_1800_5602(mem), [0x88, 0x89, 0x8a, 0x98, 0x9a]); // FDC PLx45D
    }
    const os = URL_OPTS.get('os');
    if (os === 'spo') {                                             // SM SPO
        await mon.exec('disk 0 sm1800/SM_SPO.dsk');
        await mon.exec('disk 1 sm1800/4.098.056DUBL.bin');
    }
    else if (os === 'cpm')                                          // SM OS-1800
        if (monid) console.error('OS-1800 depends on MONID 1.0 and FDC PLx45D');
        else {
            await mon.exec('disk 0 sm1800/SM_CPM.dsk');
