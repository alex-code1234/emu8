    ...
    bclick = e => {
        ...
        switch (btn.id) {
            case '0':  // STATE selector
                state = (state + 1) % 6;
                stl.backgroundImage = `url(pdp8/Pos${state + 1}.bmp)`;
                break;
            ...
            case '2':  // SW
                sw_reg = sw_reg ? 0 : 1;
                stl.backgroundImage = sw_reg ? 'url(pdp8/Up1.bmp)' : 'url(pdp8/Down1.bmp)';
                break;
            case '3':  // 0..11
            ...
            case '15': // ADDR
                ...
                regs[PC] = regs[SR];
                stl.backgroundImage = 'url(pdp8/Down2.bmp)';
                setTimeout(() => stl.backgroundImage = 'url(pdp8/Up2.bmp)', 200);
                break;
            case '16': // EXT ADDR
                if (lock || cpu.RUN) break;
                const srr = regs[SR];
                regs[DF] = (srr & 0o70) >> 3; regs[IF] = regs[IB] = srr & 0o7;
                stl.backgroundImage = 'url(pdp8/Down1.bmp)';
                setTimeout(() => stl.backgroundImage = 'url(pdp8/Up1.bmp)', 200);
                break;
            case '17': // CLEAR
                if (lock || cpu.RUN) break;
                cpu.cpu.reset(false);
            ...
            case '20': // HALT
                ...
                stl.backgroundImage = halt ? 'url(pdp8/Down1.bmp)' : 'url(pdp8/Up1.bmp)';
                break;
            case '21': // STEP
                if (lock) break;
                sstep = sstep ? 0 : 1;
                if (sstep) cpu.RUN = false;
                stl.backgroundImage = sstep ? 'url(pdp8/Down2.bmp)' : 'url(pdp8/Up2.bmp)';
                break;
            case '22': // DEP
                if (lock || cpu.RUN) break;
                memo.wr(regs[PC], regs[SR]); regs[PC] = regs[PC] + 1 & 0o7777;
                stl.backgroundImage = 'url(pdp8/Up1.bmp)';
                setTimeout(() => stl.backgroundImage = 'url(pdp8/Down1.bmp)', 200);
                break;
        }
    },
...
.fpled { position: absolute; width: 0.9%; height: 1.7%; border-radius: 20px;
    ...
    `);
    const img = await loadImage('pdp8/panel.png'),
    ...
    btn(67.585, 48.2,   0,  'pdp8/Pos1.bmp', 4.45);      // STATE/STATUS/ETC
    btn(6.95,   75.2,   1,  null,       4.45, '\u2501'); // ON/LOCK
    btn(17.32,  75.198, 2,  'pdp8/Down1.bmp');           // SW
    btn(23.45,  75.198, 3,  'pdp8/Down2.bmp'); btn(26.6,  75.198, 4,  'pdp8/Down2.bmp'); // 0 1
    btn(29.75,  75.198, 5,  'pdp8/Down2.bmp'); btn(32.9,  75.198, 6,  'pdp8/Down1.bmp'); // 2 3
    btn(36,     75.198, 7,  'pdp8/Down1.bmp'); btn(39.15, 75.198, 8,  'pdp8/Down1.bmp'); // 4 5
    btn(42.3,   75.198, 9,  'pdp8/Down2.bmp'); btn(45.45, 75.198, 10, 'pdp8/Down2.bmp'); // 6 7
    btn(48.55,  75.198, 11, 'pdp8/Down2.bmp'); btn(51.7,  75.198, 12, 'pdp8/Down1.bmp'); // 8 9
    btn(54.85,  75.198, 13, 'pdp8/Down1.bmp'); btn(57.95, 75.198, 14, 'pdp8/Down1.bmp'); // 10 11
    btn(64.45,  75.198, 15, 'pdp8/Up2.bmp');   // ADDR
    btn(67.6,   75.198, 16, 'pdp8/Up1.bmp');   // EXT ADDR
    btn(73.5,   75.198, 17, 'pdp8/Up2.bmp');   // CLEAR
    btn(76.65,  75.198, 18, 'pdp8/Up1.bmp');   // CONT
    btn(79.75,  75.198, 19, 'pdp8/Up2.bmp');   // EXAM
    btn(82.9,   75.198, 20, 'pdp8/Up1.bmp');   // HALT
    btn(86,     75.198, 21, 'pdp8/Up2.bmp');   // STEP
    btn(92.05,  75.198, 22, 'pdp8/Down1.bmp'); // DEP
